...
export function Header() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-rose/15 bg-ivory/70 text-wine shadow-[0_8px_30px_rgb(100_28_49/4%)] backdrop-blur-none">
      <div className="container-site flex h-20 items-center justify-between">
        ...
        <nav className="hidden items-center gap-7 lg:flex" aria-label="Navigation principale">
          {links.map(([label, href]) => (
            <a key={href} href={href} className="text-lg font-medium text-wine/70 transition hover:text-wine">
              {label}
            </a>
          ))}
